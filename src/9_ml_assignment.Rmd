...
match_dir = '/Users/gabrielasanchezortiz/Desktop/School/ESS/ESS 523c/yojoa-rs-secchi-demo/data/matchups'
```
...
prepData = function(df) {
  ...
  #Add ratios then trim to need to columns to speed up run
  df_prep %>% 
...
train-rmse: 0.600290 and val-rmse: 0.774694
...
The parameter that made the most difference was the gamma.
...
moreDataPrep <- function(df) {
  
  # add the three spectral indices
  df %>%
    mutate(CIG = med_Nir_corr/med_Green_corr,
           NDTI = (med_Red_corr-med_Green_corr)/(med_Red_corr+med_Green_corr),
           CVI = (med_Nir_corr*med_Red_corr)/(med_Green_corr**2.0))
}

sameDay <- moreDataPrep(sameDay)
oneDay <- moreDataPrep(oneDay)
threeDay <- moreDataPrep(threeDay)
fiveDay <- moreDataPrep(fiveDay)
...
# adding and formating data to work with xgboost
more_band_met5_feats <- c('med_Blue_corr', 'med_Green_corr', 'med_Red_corr', 'med_Nir_corr', 'NR', 'BG', 'BR', 'tot_sol_rad_KJpm2_5', 'max_temp_degK_5', 'min_temp_degK_5', 'tot_precip_m_5', 'mean_wind_mps_5', 'CIG', 'NDTI', 'CVI')

dtrain_fd_bm5 <- xgb.DMatrix(data = as.matrix(train_fiveDay[,more_band_met5_feats]), 
                      label = train_fiveDay[,target])

dtest_fd_bm5 <- xgb.DMatrix(data = as.matrix(test_fiveDay[,more_band_met5_feats]), 
                     label = test_fiveDay[,target])

# we are using the same parameters we used previously and running the model with those parameters 
params <- list(booster = "gbtree", 
               objective = "reg:squarederror", 
               eta=0.3, 
               gamma=0.5, 
...
```{r}
new_preds_fd_bm5 <- test_fiveDay %>% mutate(predicted_fd_bm5 = predict(xgb_naive_fd_bm5, dtest_fd_bm5))

new_evals_fd_bm5 <- preds_fd_bm5 %>%
  summarise(rmse = rmse(secchi, predicted_fd_bm5),
            mae = mae(secchi, predicted_fd_bm5),
            mape = mape(secchi, predicted_fd_bm5),
            bias = bias(secchi, predicted_fd_bm5),
...
```{r}
# Adding the three spectral indices did not improve my results, but it might be due to an error I could have made.
```
...
ggplot(test_fiveDay, aes(x = secchi, y = secchi_pred)) +
  geom_point() +
  geom_smooth(method = 'lm', color = 'blue', se = F) + # line of best fit of data vs fitted data
  geom_abline(intercept = 0, slope = 1, color = 'black') + # one to one line
  ggthemes::theme_few()
```
